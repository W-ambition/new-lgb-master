      ...
      <template #action="{ record }">
        <TableAction
          ...
          :actions="[
            ...
            { label: '产品', onClick: handlePruduct.bind(null, record) },
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const { PId } = getAuthCache<UserInfo>(USER_INFO_KEY);
      ...
      function goToLottery(id, name, type) {
        go(`lotteryListInWareHouse?WareHouseId=${id}&WareHouseName=${name}&LevelType=${type}&StockType=${2}`);
      }
