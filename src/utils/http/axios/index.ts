...
const transform: AxiosTransform = {
  ...
  transformRequestHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // 接口请求903,token失效，跳转到登录页面
    if (Code === ResultEnum.ERROR) {
      const tokenInvalidMsg = '登录状态失效';
      createMessage.error(tokenInvalidMsg);
      userStore.confirmLoginOut(false);
      Promise.reject(new Error(tokenInvalidMsg));
      return errorResult;
    }
    // 登录超时
