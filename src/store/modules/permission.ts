...
import { LOGIN_KEY } from '/@/enums/cacheEnum';
...
export const usePermissionStore = defineStore({
  ...
  getters: {
    getLoginInfo(): LoginInfo {
      return getAuthCache<LoginInfo>(LOGIN_KEY);
    },
    getPermCodeList() {
  ...
  actions: {
    ...
    async changePermissionCode(userId: string) {
      const codeList = await getPermCodeByUserId({ UserId: userId });
      console.log(codeList)
    ...
    async buildRoutesAction(id?: number | string): Promise<AppRouteRecordRaw[]> {
      ...
      if (permissionMode === PermissionModeEnum.ROLE) {
        ...
        try {
          const { UserId } = this.getLoginInfo;
          this.changePermissionCode(UserId);
